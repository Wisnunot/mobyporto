...
<div class="bg-white border-b border-gray-200 py-4">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav class="flex text-sm font-medium text-gray-500">
            <a href="{{ route('cars.index') }}" class="hover:text-primary transition-colors">{{ __('ui.showroom') }}</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12">
        ...
        <div class="lg:col-span-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-10">
                <h3 class="text-2xl font-bold text-primary mb-6">{{ __('ui.specifications') }}</h3>
                
                <div class="grid grid-cols-2 md:grid-cols-3 gap-y-8 gap-x-6">
                    <div>
                        <div class="text-sm text-gray-500 mb-1">{{ __('ui.brand') }}</div>
                        <div class="font-semibold text-primary text-lg">{{ $car->brand }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-500 mb-1">{{ __('ui.model') }}</div>
                        <div class="font-semibold text-primary text-lg">{{ $car->title }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-500 mb-1">{{ __('ui.year') }}</div>
                        <div class="font-semibold text-primary text-lg">{{ $car->year }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-500 mb-1">{{ __('ui.mileage') }}</div>
                        <div class="font-semibold text-primary text-lg">{{ number_format($car->mileage, 0, ',', '.') }} km</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-500 mb-1">{{ __('ui.transmission') }}</div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                <h3 class="text-2xl font-bold text-primary mb-6">{{ __('ui.description') }}</h3>
        ...
        <div class="lg:col-span-4">
            <div class="sticky top-28 bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
                ...
                <div class="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">{{ __('ui.price') }}</div>
...
<div class="bg-slate-50 py-16 border-t border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-end mb-10">
            <div>
                <h3 class="text-3xl font-extrabold text-primary">{{ __('ui.similar_vehicles') }}</h3>
                <p class="text-gray-500 mt-2">{{ __('ui.similar_desc') }}</p>
            </div>
